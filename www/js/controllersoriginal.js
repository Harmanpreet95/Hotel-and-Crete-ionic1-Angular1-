       ...
       $http.post(link, postData).then(function(resp){
             ...
             //alert(JSON.stringify(resp));             
                if(resp.data.status==true)
                {
                    ...
                    $state.go('menu.groupHotel',{},{reload:true});
                      var alertPopup = $ionicPopup.alert({
...
.controller('loginCtrl',
    function($scope,$state,$stateParams,$http,$log,Base_Url,$window,$ionicLoading,$rootScope,$ionicPopup,$httpParamSerializer,$httpParamSerializerJQLike)
   {
...
var jsonData=angular.toJson($scope.data);
    ...
    //    $scope.serial = $httpParamSerializerJQLike({email : $scope.data.email,password:$scope.data.password});
    //     $scope.headers = {
...
//             //    $http.post(link,$scope.serial,$scope.headers).success(function(resp)
//             //     {
...
var weekday = new Array(7);
    ...
    if(today.getDay() == 6){
     ...
     // console.log( $rootScope.today_day )
        ...
        $rootScope.tomo_date = $filter('date')(tomorrow.setDate(tomorrow.getDate()+1),'MM/dd/yyyy');

        var tomorrow = new Date();
        $rootScope.next_day   = weekday[1];
        $rootScope.next_date = $filter('date')( tomorrow.setDate(tomorrow.getDate()+2),'MM/dd/yyyy');


                var tomorrow = new Date();
        $rootScope.next_nextday   = weekday[2];
        $rootScope.next_date = $filter('date')( tomorrow.setDate(tomorrow.getDate()+3),'MM/dd/yyyy');
...
$ionicLoading.show();
    ...
    $scope.xyz = function(descID,title_main){
...
//               alert('harman')
//               alert($rootScope.gastro.length);
                $scope.crete_id =resp.result[i].CreteDetail[j].Gastronomy[k].cretedetailid;
                console.log($scope.crete_id);
            
               if(id == $scope.crete_id){
              //     alert("if1")
               $rootScope.crete_gastronomy = resp.result[i].CreteDetail[j].Gastronomy;
...
//    alert($rootScope.description);
   // $state.go('menu.birthofzeus');
    ...
    if(index_value == 0){
            $state.go('menu.birthofzeus',{},{reload:true}); 
        }
        else if(index_value == 1){
            $state.go('menu.product',{},{reload:true});
        }else{
            $state.go('menu.product',{},{reload:true});
        }
})
...
$scope.guidevillage = function(creteid,title,image){
    $ionicLoading.show();
    ...
    $http.post(link,$scope.beaches).success(function(resp){
      ...
      console.log($rootScope.title1);
        ...
        for(var x = 0 ; x <= resp.result.length ; x++){
            ...
            console.log(resp.result.length);
            for(var k=0;k<=resp.result[x].GuideData.length;k++){
                resp.result[x].GuideData[k].image = resp.result[x].GuideData[k].image.split(',')[0];
                $rootScope.beach1 = resp.result[x].GuideData;
...
.controller('beachesCtrl', function ($scope, $stateParams,$rootScope,$state,$window,Base_Url,$http,$ionicLoading) {
 ...
 $scope.cre_id = localStorage.getItem('creteidbeach');
  ...
  $scope.beach_info = function(beach_id,lat,long,description,title,image){
    //  alert("srishtiiii")
    ...
    $http.post(link,$scope.beaches).success(function(resp){
      ...
      console.log($rootScope.title1);
        ...
        for(var x = 0 ; x <= resp.result.length ; x++){
            ...
            console.log(resp.result.length);
            for(var k=0;k<=resp.result[x].GuideData.length;k++){
                console.log("beachid - >" + beach_id);
                console.log("guidecreteid ->" + resp.result[x].GuideData[k].id);
                if(beach_id== resp.result[x].GuideData[k].id){
                    // alert("if");
                console.log(resp.result[x].GuideData[k].image);
                $scope.image= resp.result[x].GuideData[k].image.split(',');
...
.controller('beachareanoethCtrl', function ($scope, $stateParams,$rootScope,$state,$window,Base_Url,$http,$cordovaGeolocation,$cordovaLaunchNavigator) {
    ...
    $http.post(link,$scope.beachnorth).success(function(response){
       ...
       {
           console.log(response.result[a].GuideBeache.image);
           response.result[a].GuideBeache.image = response.result[a].GuideBeache.image.split(',')[0];
...
.controller('coastCtrl', function ($scope, $stateParams,$rootScope,$state,$window,Base_Url,$http,$cordovaGeolocation,$cordovaLaunchNavigator) {
